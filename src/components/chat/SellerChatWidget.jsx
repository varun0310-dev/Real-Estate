...
/**
 * SellerChatWidget – renders for seller / superadmin users.
 * Shows a floating "Inbox" bubble with total unread count.
 * Clicking it opens a conversation picker, then clicking a conversation
 * opens FloatingChat for that conversation.
 */
export default function SellerChatWidget({ currentUser }) {
    ...
    const isSeller = currentUser?.userType === 'seller' || currentUser?.userType === 'superadmin';
                ...
                socket.on('new_message', (msg) => {
                    setConversations((prev) => {
                        ...
                        if (idx !== -1) {
                            ...
                            // Increment unread only if not the active conversation
                            const current = activeConvRef.current;
                            if (msg.senderId._id !== currentUser._id && (!current || current._id !== msg.conversationId)) {
                                newConvs[idx] = { ...newConvs[idx], sellerUnreadCount: (newConvs[idx].sellerUnreadCount || 0) + 1 };
                            }
    ...
    const handleOpenConv = (conv) => {
        ...
        setConversations((prev) => {
            ...
            if (idx !== -1) {
                const delta = newConvs[idx].sellerUnreadCount || 0;
                newConvs[idx] = { ...newConvs[idx], sellerUnreadCount: 0 };
    ...
    return (
        <>
            ...
            {/* Only show the inbox bubble when no chat is open OR chat is minimized */}
            {!activeConv && (
                            ...
                            <div className="max-h-72 overflow-y-auto divide-y divide-gray-50">
                                {loading ? (
                                    ...
                                    conversations.map((conv) => {
                                        ...
                                        const unread = conv.sellerUnreadCount || 0;
                                        const propertyTitle = conv.propertyId?.title;
                                        const hasPhoto = buyer?.profileImage && buyer.profileImage.trim() !== '';
                                        return (
                                            <div
                                                key={conv._id}
                                            ...
                                            >
                                                ...
                                                <div className="w-11 h-11 rounded-full bg-gradient-to-br from-[#4960B2] to-[#6c7fcf] flex items-center justify-center flex-shrink-0 overflow-hidden shadow-sm">
                                                    {hasPhoto
                                                        ...
                                                        : <span className="text-white font-bold text-base">{buyer?.name?.charAt(0).toUpperCase() || '?'}</span>
                                                    }
                                                </div>
